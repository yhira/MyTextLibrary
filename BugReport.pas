...
uses
  ...
  Dialogs, StdCtrls, Clipbrd, MAIN, ComCtrls, ShellAPI;
...
begin
  BugLabel.Caption := Application.Title + 'を使用して起きた不具合は、以下のテンプレートを' +
    '使用して下記URL' + #13#10 + 'まで報告して下さい。出来る限り対応します。';
  BugMemo.Clear;
//  WebPageLabel.Caption := '';

  Partition := '---------------------------------------------------';
  Day := FormatDateTime('yyyy/mm/dd',Now);
  Title := Application.Title + ' ' + VER_INFO + ' ' + 'バグレポート' + ' ' + Day;
  OSKind := OSPlatformInfo;
  ...
  BugMemo.Lines.Add('【ItemCount】 ' + IntToStr(MainForm.TreeView.Items.Count));
  BugMemo.Lines.Add(Partition);
